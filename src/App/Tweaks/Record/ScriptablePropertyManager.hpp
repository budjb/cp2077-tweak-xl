...
public:
     ...
     * property getter.
     */
    virtual Red::CName GetFunctionHash(const ScriptableRecordSpecPtr& aRecordSpec,
     ...
     */
    std::string GetFunctionBaseName(const std::string& aName) const;
     ...
     * name.
     */
    std::string GetFunctionName(const std::string& aName) const;
...
protected:
    ...
    static std::optional<Red::DynArray<THandle<Red::TweakDBRecord>>> GetRecordArray(const Red::Value<>& aValue,
...
/**
 * @brief A property getter that retrieve an array of foreign keys to TweakDB records from a scriptable record property.
 *
...
/**
 * @brief A property getter that check whether a TweakDB record is contained within an array of foreign keys to TweakDB
...
/**
 * @brief A property getter that return a foreign key to a TweakDB record from an array of TweakDB records from
...
/**
 * @brief A property getter that retrieve a foreign key to a TweakDB record from a scriptable record property.
 *
...
/**
 * @brief A property getter that retrieve a foreign key to a TweakDB record from a scriptable
...
/**
 * @brief A property getter that retrieve the count of items in an array associated with a scriptable record property.
 *
 ...
 * result is validated to ensure that it is an array. On success, the getter will return the count of items in the
 * array.
 */
...
/**
 * @brief A property getter that retrieve an item at a specific index from an array associated with a scriptable record
 * property.
 *
 * The generated function name for this getter has the prefix @c Get and the suffix @c Item. For example, a property
 * named @c MyProperty would correspond to a function named @c GetMyPropertyItem.
 ...
 * are of the expected type as defined by the property specification. On success, the getter will return the item at
...
/**
 * @brief A property getter that check whether a specific item is contained within an array associated with a scriptable
 * record property.
 *
 ...
 * The TweakDB flat associated with the property is expected to contain an array of any type other than foreign keys to
 * other TweakDB records. During processing, the result is validated to ensure that it is an array and that its elements
 * are of the expected type as defined by the property specification. The getter takes an item as an argument and
...
/**
 * @brief A property getter that retrieve an array of resource references from a scriptable record property.
 *
 * The generated function name for this getter has no additional prefix or suffix beyond the base property name. For
 * example, a property named @c MyProperty would correspond to a function named @c MyProperty.
 *
...
/**
 * @brief A property getter that retrieve an item at a specific index from an array of resource references associated
...
/**
 * @brief A property getter that retrieve a resource reference from a scriptable record property.
 *
 * The generated function name for this getter has no additional prefix or suffix beyond the base property name. For
 * example, a property named @c MyProperty would correspond to a function named @c MyProperty.
 *
...
/**
 * @brief A registry for handling the registration and invocation of script functions that serve as property getters for
...
public:
     ...
     *
     * This process relies on a property of a scriptable record having previously been provided to the
     * @c RegisterScriptableProperty function so that the hash of the function's signature can be mapped to the
     * appropriate getter type.
     *
     * @param aRecordSpec The specification of the scriptable record type that this function belongs to.
...
private:
     ...
     * call stack frame. From this context, the handler can determine which specialized getter type may service the
     * request.
     *
     ...
     * function completes, the stack frame will be advanced past the getter type.
     *
     ...
     */
    static std::span<const GetterType> GetGetterTypes(const ScriptablePropertySpecPtr& aPropSpec);
     ...
     * specification. This involves removing the function's entry from the registry and freeing the memory allocated for
     * its bytecode.
     *
     ...
     * entry.
     */
    static void TruncateByteCode(const Core::SharedPtr<FunctionEntry>& aEntry);
