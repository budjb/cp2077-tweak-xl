...
public:
    ...
    virtual void ConfigureScriptFunction(Red::CClassFunction* aFunction,
                                         const ScriptablePropertySpecPtr& aPropSpec) const = 0;
    static void NoOpFunction(Red::IScriptable*, Red::CStackFrame*, void*, int64_t);
#endif
...
class ScriptablePropertyHandler
    : Core::LoggingAgent
    , Core::ShareFromThis<ScriptablePropertyHandler>
{
public:
    /**
     * @brief Constructs a ScriptablePropertyHandler instance with the given TweakDB manager.
     *
     * @param aManager A deferred pointer to the TweakDB manager used to retrieve scriptable record property values from
     * TweakDB at runtime.
     */
    explicit ScriptablePropertyHandler(const Core::DeferredPtr<Red::TweakDBManager>& aManager);
...
private:
    ...
    static GetterType GetGetterType(Red::CStackFrame* aFrame);
