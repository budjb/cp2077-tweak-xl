                                 ...
                                 const Core::SharedPtr<App::TweakChangelog>& aChangelog)
{
    ...
    for (const auto& [recordId, recordEntry] : m_pendingRecords)
    {
        ...
        for (const auto& propInfo : recordInfo->GetProperties() | std::views::values)
        {
            ...
            {
                auto targetPropId = recordId + propInfo->GetFlatSuffix();
...

    {
        ...
        if (aChangelog)
        {
            for (const auto& flatId : aManager->GetFlats(batch))
            {
                ...
                if (!flatOld.instance)
                {
                    LogInfo("Old flat did not exist for {}.", aManager->GetName(flatId));
                    flatOld = aManager->GetDefault(flatNew.type);
                }
                else if (flatOld.type != flatNew.type)
                {
                    LogWarning("Type mismatch for {} assignment. {} != {}", aManager->GetName(flatId), flatOld.type->GetName().ToString(), flatNew.type->GetName().ToString());
                    continue;
                }
