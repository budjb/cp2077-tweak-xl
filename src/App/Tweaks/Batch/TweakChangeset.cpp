                                 ...
                                 const Core::SharedPtr<App::TweakChangelog>& aChangelog)
{
                ...
                for (const auto& descendantId : aManager->GetReflection()->GetOriginalDescendants(sourceId))
                {
                    ...
                    if (isAssignment)
                    {
                        ...
                        if (descendantFlatValue == sourceFlatValue)
                        {
                            ...
                            clonedAssignment.value = Red::ERTDBFlatType::Construct(clonedAssignment.type);
                            clonedAssignment.type->Assign(clonedAssignment.value.get(), sourceAssignment.value.get());

                            UpdateRecord(descendantId);
                        }
                        else if (Red::ERTDBFlatType::IsArrayType(sourceFlatValue.type))
                        {
                            if (!isConvertedToMutation)
                            {
                                ...
                                for (uint32_t sourceIndex = 0; sourceIndex < sourceLength; ++sourceIndex)
                                {
                                    ...
                                    auto clonedValue = Red::ERTDBFlatType::Construct(elementType);
...

    {
        ...
        for (const auto& item : m_pendingFlats)
        {
            ...
            if (aChangelog)
            {
                ...
                else if (Red::ERTDBFlatType::IsResRefToken(flatType))
                {
                ...
                else if (Red::ERTDBFlatType::IsResRefTokenArray(flatType))
                {
    ...
    for (const auto& [flatId, mutation] : m_pendingMutations)
    {
        ...
        if (!flatData.instance)
        {
            ...
            flatData.type = Red::ERTDBFlatType::GetArrayType(elementType);
        }
        ...
        auto targetArray = Red::ERTDBFlatType::Construct(targetType);
        ...
        if (aChangelog)
        {
            const auto isForeignKey = Red::ERTDBFlatType::IsForeignKeyArray(targetType);
