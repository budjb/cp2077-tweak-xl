...
const Red::TweakDBRecordInfo* Red::TweakDBReflection::FindRecordInfo(const uint32_t aHash)
{
...
const Red::TweakDBRecordInfo* Red::TweakDBReflection::FindCustomRecordInfo(const uint32_t aHash)
{
    if (const auto* recordInfo = FindRecordInfo(aHash); recordInfo && recordInfo->IsCustom())
        return recordInfo;
    return nullptr;
}

const Red::TweakDBRecordInfo* Red::TweakDBReflection::FindRecordInfo(const CClass* aType)
{
    std::shared_lock lockR(m_mutex);
    if (const auto iter = m_recordInfoByCName.find(aType->GetName()); iter != m_recordInfoByCName.end())
        return iter->second.get();
    return nullptr;
}
                                                                        ...
                                                                        Red::TweakDBID aSampleId)
{
    ...
    for (uint32_t funcIndex = 0u; funcIndex < aType->funcs.Size(); ++funcIndex)
    {
        ...
        if (!func->returnType)
        {
            ...
            propInfo->SetType(Red::ERTDBFlatType::TweakDBIDArray);
            propInfo->SetArray();
            propInfo->SetElementType(Red::ERTDBFlatType::TweakDBID);
        ...
        else
        {
            ...
            case Red::rtti::ERTTIType::WeakHandle:
            {
                ...
                propInfo->SetType(Red::ERTDBFlatType::TweakDBID);
                propInfo->SetForeignKey();
                propInfo->SetForeignType(recordType);
            ...
            case Red::rtti::ERTTIType::Array:
            {
                ...
                {
                    propInfo->SetType(Red::ERTDBFlatType::ResRefArray);
                    propInfo->SetArray();
                    propInfo->SetElementType(Red::ERTDBFlatType::ResRef);
...

    {
        ...
        if (extraFlatsIt != s_extraFlats.end())
        {
            for (const auto& extraFlat : extraFlatsIt.value())
            {
                ...
                propInfo->SetType(extraFlat.typeName);

                if (propInfo->GetType().GetType()->GetType() == Red::rtti::ERTTIType::Array)
                {
                    const auto arrayType = reinterpret_cast<const Red::CRTTIArrayType*>(propInfo->GetType().GetType());
...
{
    const Red::rtti::IType* type = GetRTTI()->GetType(aTypeName);
    ...
    const Core::SharedPtr<Red::TweakDBRecordInfo>& aRecordInfo)
{
    ...
    if (auto [it, success] = m_recordInfoByCName.insert({aRecordInfo->GetType(), aRecordInfo}); success)
    {
        const auto record = it->second;
        m_recordInfoByHash[record->GetTypeHash()] = record;
        return record;
    }
    return nullptr;
}
